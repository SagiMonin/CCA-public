...
if __name__ == '__main__':
    ...
    bs_params = {
        'lam': 0.5,  # The strength of the smoothness parameter
        ...
        'cg_maxiter': 80  # The number of PCG iterations
    }
    ...
    directory_name = 'Results_qualtitive'
    img_name = '055'

    #resmat = spio.loadmat(os.path.join(directory_name, img_name))
    for idx in range(15):
        path_file = 'data_im_sgm'+ str(idx+1) + '.mat'
        resmat = spio.loadmat(path_file)

        target = resmat['curr_dispar']
        confidence = resmat['conf_score'] # Other herustics
        # confidence2 = resmat['conf_score2'] # Both
        # confidence3 = resmat['conf_score_no_suprress'] # Just score
        ...
        mdic = {"output_solver" : output_solver}
        spio.savemat('result_filter_SGM' + str(idx+1) + '.mat', mdic)
    # output_solver2 = output_solver - np.min(output_solver)
    # output_solver2 = output_solver2/np.max(output_solver2)
    # imsave('bilateral_solver_result_conf1_2.png',(255 * output_solver2).astype(np.uint8))



    # plt.imshow(output_solver2)
    # plt.show()
    # print('hi')
    #np.clip(output_solver, 0, 1, output_solver)

    #np.clip(output_solver2, 0, 1, output_solver2)

   # imsave(os.path.join(directory_name, img_name, 'bilateral_solver_result.png'),
    #       (255 * output_solver).astype(np.uint8))
  #  imsave('bilateral_solver_result.png',(255 * output_solver2).astype(np.uint8))
