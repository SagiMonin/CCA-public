...
function [D1,D2,score_out] = stereoMatchWindowCensus(I1, I2, window_radius, disparity_range)
...
score_out = zeros(size(D1,1),size(D1,2),length(disparity_range(1):disparity_range(2)));
idx_d = 0;
for d = disparity_range(1):disparity_range(2)
    ...
    D1(:,range1) = D1crop;
    D2(:,range2) = D2crop;
    score_out(:,:,idx_d) = [zeros(size(S,1),d),S];
