...
if __name__ == '__main__':
    ...
    bs_params = {
        'lam': 15,  # The strength of the smoothness parameter
    ...
    # directory_name = '..\\..\\results_CSGM\\results_DP_pixel\\res_no_filt_encc3'
    # directory_name_save =  '..\\..\\results_CSGM\\results_DP_pixel\\res_filt_encc3'
    for idx in range(1):
        img_name = 'im' + str(idx+1)
        # resmat = spio.loadmat(os.path.join(directory_name, img_name+'.mat'))
        #resmat = spio.loadmat(('phone_dispar2_test_bilateral.mat'))
        resmat= spio.loadmat('im1.mat')
        # Full size image
        target = resmat['curr_dispar']
        confidence1 = resmat['conf_score']
        # confidence2 = resmat['conf_score2']
        # confidence3 = resmat['conf_score3']
        reference = resmat['im_guide']
        ...
        c = confidence1.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        output_solver2 = output_solver - np.min(output_solver)
        output_solver2 = output_solver2/np.max(output_solver2)
        imsave((os.path.join(img_name + 'full_filt1.png')),(255 * output_solver2).astype(np.uint8))

        # c = confidence2.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'full_filt2.png')), (255 * output_solver2).astype(np.uint8))
        #
        # c = confidence3.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'full_filt3.png')), (255 * output_solver2).astype(np.uint8))

        # Small size image
        # target = resmat['curr_dispar_small']
        # confidence1 = resmat['conf_score_small_1']
        # confidence2 = resmat['conf_score_small_2']
        # confidence3 = resmat['conf_score_small_3']
        # reference = resmat['im_guide_small']
        #
        # # Construct a hard bilateral grid
        # im_shape = reference.shape[:2]
        # grid = BilateralGrid(reference, **grid_params)
        ...
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2/np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'small_filt1.png')),(255 * output_solver2).astype(np.uint8))
        #
        # c = confidence2.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'small_filt2.png')), (255 * output_solver2).astype(np.uint8))
        #
        # c = confidence3.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'small_filt3.png')), (255 * output_solver2).astype(np.uint8))

        # DN size image
        # target = resmat['curr_dispar_dn']
        # confidence1 = resmat['conf_score_dn_1']
        # confidence2 = resmat['conf_score_dn_2']
        # confidence3 = resmat['conf_score_dn_3']
        # reference = resmat['im_guide_small']
        #
        # #
        ...
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'dn_filt1.png')),
        #        (255 * output_solver2).astype(np.uint8))
        #
        # c = confidence2.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'dn_filt2.png')),
        #        (255 * output_solver2).astype(np.uint8))
        #
        # c = confidence3.reshape(-1, 1).astype(np.double)  # / (pow(2,16)-1)
        # output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
        # output_solver2 = output_solver - np.min(output_solver)
        # output_solver2 = output_solver2 / np.max(output_solver2)
        # imsave((os.path.join(directory_name_save, img_name + 'dn_filt3.png')),
        #        (255 * output_solver2).astype(np.uint8))