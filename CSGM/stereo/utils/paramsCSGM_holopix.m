function params = paramsCSGM_holopix()
% Configurable parameters for algorithm 
flagLRC = 0; % LRC processing - for phone-cameras
flagPost = 0; % Post processing 

params.dispRange = [-2,0]; % Disparity range - (this is for first level of pyramid) for DP this is symmetric 
params.numIter = [0] ; % Number of iterations 

% params.subpix = ''; 'ENCC'; 

params.cost = 'BT'; % Cost function type
params.gaussKerSigma = 5;% STD of gaussian for average-weighting of score

params.confidenceThresh=-6e-3; % Threshold for flatting parabolas %%% Tune-flag
...
params.P1param = 3; %  % Penalty for large diff in dispairty
params.sigmaEdges = 8; % Used to identify edges to give less penalty for smoothness
...
params.priorW = 0.01;0.045; % This is prior when jumping from low level to higher level in the pyramid.
params.idx_pyr = 1; % First level of pyramid - this stays 1
 
%% ENNC - parameter
params.encc_window = 7;

%% Sub-pixel interpolation - This is based on simulations Sagi.K did to check pixel-locking
params.interpolant = 'parabola';% ['f1', 'ENCC'] % Interpolation kernel for sub-pixel estimation
