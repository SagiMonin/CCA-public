function [ disparity_map,C ] = SGMWrapper( im_left, im_right, params )
...
Lr = costAggregation(C(:,:,2:end-1), P1, P2, directions_num);
...
disparity_map = disparity_map + disparity_range(1);
...
idx_score = 1;
if 0
    for idx_dispar = disparity_range(1) : disparity_range(2)
    ...
    %         score(:,:,idx_score) = imgaussfilt(-abs(im_L - imtranslate(im_R,[-idx_dispar,0],'linear','OutputView' ,'same','FillValues',0)),params.gaussKerSigma,'Padding','symmetric');
        C(:,:,idx_score) = conv2(abs(im_support - imtranslate(im_reference,[-idx_dispar,0],'linear','OutputView' ,'same','FillValues',0)),filt,'same');
    ...
    for idx_dispar = disparity_range(1) : disparity_range(2)
        ...
        C(:,:,idx_score) = conv2(min(cat(3,A,B),[],3),filt,'same');
        idx_score = idx_score + 1;
