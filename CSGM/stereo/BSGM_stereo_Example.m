...
%     whos imgL imgR
%     [h,w,cn] = size(imgL);
    imgL = uint8(kron(rand(64)*255,ones(4)));
    imgR = imtranslate(imgL,[-3,0]);
    ...
    win_size = 3;
    min_disp = 0;16; %16;             % 0
    num_disp = 16*2-min_disp; % fix(w/8) + 15
    ...
    stereo = cv.StereoSGBM('MinDisparity',min_disp, 'NumDisparities',num_disp, ...
        'BlockSize',1, 'P1',5, 'P2',64, ...
        'Disp12MaxDiff',1, 'UniquenessRatio',0, ...
        'SpeckleWindowSize',0, 'SpeckleRange',0, 'Mode','HH');
    display(stereo)
...
%     DD = (D - stereo.MinDisparity) / stereo.NumDisparities; % normalized [0,1]
    figure(1);
    subplot(121); imshow(D,[]), %colorbar, title('Disparity')
    
    %% SGM
    D2 = disparity(imgR,imgL,'DisparityRange',[0 16]);
    subplot(122);  imshow(D2,[]), %colorbar, title('Disparity')
