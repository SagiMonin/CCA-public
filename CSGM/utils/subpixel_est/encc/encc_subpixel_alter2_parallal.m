function [ tau]=encc_subpixel_alter2_parallal(L, R, window, maxDisp, flag_zm, dispar_map)
...
VaL = real(sqrt(VarL - Mol.^2));
VaR = real(sqrt(VarR - Mor.^2));
...
% LL=zeros(size(L)+[w1-1 w2]);
% RR=zeros(size(R)+[w1-1 w2]);
...
%     for j=mw2+1:sizeL(2)-mw2 %x-idx
        ...
        if j-mw2-1>maxDisp % Iteration over disparites (if max disp is not limit)
            a1 = R(i-mw1:i+mw1, j-maxDisp-mw2+1:j+mw2-maxDisp+2-1);
            crossFilter(1) = Ml(:)'*a1(:);
            a2 = R(i-mw1:i+mw1, j-maxDisp-mw2+2:j+mw2-maxDisp+2-0);
            crossFilter(2) = Ml(:)'*a2(:);
            ...
            r = rMat(i-mw1,k-mw2);
            B1= (crossFilter(k-(j-maxDisp)+2-2)-Mol(i-mw1,j-mw2)*Mor(i-mw1,k-mw2))/(VaL(i-mw1,j-mw2)*VaR(i-mw1,k-mw2)+.00001);
            B2 = (crossFilter(k-(j-maxDisp)+1-2)-Mol(i-mw1,j-mw2)*Mor(i-mw1,k-mw2-1))/(VaL(i-mw1,j-mw2)*VaR(i-mw1,k-mw2-1)+0.00001);                
