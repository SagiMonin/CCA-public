function [ tau,max_map]=encc_subpixel_alter3_2(L, R, window, maxDisp, flag_zm, flag_mode,dispar_map)
...
%             crossFilter(1) = Ml(:)'*a(:);
            ...
            for k=j+curr_disp:j+curr_disp+1

                % optimum solution 
                lambda = Lambdas(i-mw1,k-mw2);
                r = rMat(i-mw1,k-mw2);

                B1= (crossFilter(k-(j-maxDisp)+2-1)-Mol(i-mw1,j-mw2)*Mor(i-mw1,k-mw2))/(VaL(i-mw1,j-mw2)*VaR(i-mw1,k-mw2)+.00001);
                B2 = (crossFilter(k-(j-maxDisp)+1-1)-Mol(i-mw1,j-mw2)*Mor(i-mw1,k-mw2-1))/(VaL(i-mw1,j-mw2)*VaR(i-mw1,k-mw2-1)+0.00001);
