function [dispar_map, sum_parab,conf_score_no_suprress] = currPhoneLvlCSGM(im_L, im_R, im_guide, params, prior_lvl_parab, prior_lvl_dispar )
...
% im_guide = im_guide - cv.bilateralFilter(im_guide ,'Diameter',15, 'SigmaSpace', 3, 'SigmaColor', 20);

% [im_L,~]=normalizeImg(im_L,params.normKerSize);
% [im_R,~]=normalizeImg(im_R,params.normKerSize);
% im_L = (im_L-min(im_L(:))) * 255;
% im_R = (im_R-min(im_R(:))) * 255;
...
elseif strcmp(params.interpolant, 'ENCC')
    % Calculate cost and neighboors + confidence score + disparity integer value.
    [cost_neig, conf_score, dispar_int_val,conf_score_no_suprress] = disparCost(im_L, im_R, params);
%     [parab,invalid_mask] = estSubPixENCC(cost_neig, dispar_int_val, im_L_orig, im_R_orig, params);
    [parab,invalid_mask] = estSubPixENCC(cost_neig, dispar_int_val, im_L_orig, im_R_orig, params);

%     [parab, disparity_res] = subPixENCC(cost_neig, dispar_int_val, im_L, im_R, params);
