function params = paramsCSGMPhone()
% Configurable parameters for algorithm 
flagLRC = 1; % LRC processing - for phone-cameras
flagPost = 0; % Post processing 

params.dispRange = [-1 ,1]; % Disparity range - (this is for first level of pyramid) for DP this is symmetric 
params.numIter = [4,4] ; % Number of iterations 

% params.subpix = ''; 'ENCC'; 

params.cost = 'SAD'; % Cost function type
params.gaussKerSigma =11;;% STD of gaussian for average-weighting of score
% params.normKerSize = 7; % Used for image normalization

params.confidenceThresh=-0.0001; % Threshold for flatting parabolas %%% Tune-flag
...
params.P1param = 7; % Penalty for large diff in dispairty
params.sigmaEdges = 6; % Used to identify edges to give less penalty for smoothness
...
params.levels = [2];% SGM pyramid levels
params.priorW = 0.01; 0.1; % This is prior when jumping from low level to higher level in the pyramid.
params.idx_pyr = 1; % First level of pyramid - this stays 1
params.pyr_max = 2;
%% ENNC - parameter
params.encc_window = [17];
params.encc_window2 = [17];
